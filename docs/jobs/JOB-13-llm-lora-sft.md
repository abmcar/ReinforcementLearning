...
实现 LoRA SFT 训练管线,在 JOB-11 数据上跑通一次完整训练(任意 reward 目标),产出可复现的训练脚本和一份 sanity checkpoint。
...
- [ ] 训练 pipeline 跑通至少一次:**完整 1 epoch 在 ≥ 1k 样本子集上 loss 滑动平均单调不增**(若用更小子集,在 `docs/llm_sft.md` 说明并给出曲线)
- [ ] Loss 曲线落到 `docs/figures/JOB-13-loss.png`
- [ ] 推理脚本可加载 LoRA 权重并做单条 / batch 推理,SFT 前后推理结果在同一 prompt 上有可见差异(在 `docs/llm_sft.md` 给一个例子)
- [ ] 显存占用 / 训练时长实测数据在 `docs/llm_sft.md`
