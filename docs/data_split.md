...
### 1.1 划分策略选型
* **采用方案**：**全局时间点切分 (Global Time-based Split)**
* **选型理由**：推荐系统具有极强的时效性，必须防范“未来信息泄漏（Data Leakage）”。随机划分或交叉验证在实际生产环境中不可行。按照历史日志（`entry_created_at`）严格排序，可以完美模拟推荐系统在线上“用过去的数据训练，在未来的流量上验证”的真实流程。
* **划分比例**：
  * **Train (训练集)**：前 80% 时间段的日志
  * **Validation (验证集)**：中间 10% 时间段的日志
  * **Test (测试集)**：最后 10% 时间段的日志
